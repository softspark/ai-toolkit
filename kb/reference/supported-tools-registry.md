...
| Config paths | `~/.claude/settings.json`, `.claude/settings.local.json`, `CLAUDE.md`, `.claude/agents/*.md`, `.claude/skills/*/SKILL.md` |
| Our generators | — (Claude Code is the primary target; toolkit content ships directly as `.md` files and `settings.json` merges) |
| Tracked capabilities | `PreToolUse`, `PostToolUse`, `SessionStart`, `SubagentStop`, `UserPromptSubmit`, slash commands, MCP server, sub-agent, output style, `SKILL.md` |
...
| Config paths | `.cursorrules`, `.cursor/rules/*.mdc`, `.cursor/mcp.json`, `~/.cursor/mcp.json` |
| Our generators | `scripts/generate_cursor_rules.py`, `scripts/generate_cursor_mdc.py` |
| Tracked capabilities | `cursorrules`, `.cursor/rules`, `mcp.json`, Composer, Agent Mode, YOLO, auto-apply |
...
| Config paths | `.windsurfrules`, `.windsurf/rules/*.md`, `~/.codeium/windsurf/mcp_config.json` |
| Our generators | `scripts/generate_windsurf.py`, `scripts/generate_windsurf_rules.py` |
| Tracked capabilities | Cascade, `windsurfrules`, Flow Actions, MCP, memories |
...
| Config paths | `.github/copilot-instructions.md`, `.github/mcp.json`, `~/.copilot/mcp-config.json` |
| Our generators | `scripts/generate_copilot.py` |
| Tracked capabilities | `copilot-instructions.md`, Copilot Chat, Copilot Workspace, MCP, agents, skillset |
...
| Config paths | `GEMINI.md`, `.gemini/settings.json`, `~/.gemini/settings.json` |
| Our generators | `scripts/generate_gemini.py` |
| Tracked capabilities | `GEMINI.md`, `mcpServers`, tools, `settings.json` |
...
| Config paths | `.clinerules/*.md`, `~/.cline/data/settings/cline_mcp_settings.json` |
| Our generators | `scripts/generate_cline.py`, `scripts/generate_cline_rules.py` |
| Tracked capabilities | `clinerules`, Plan Mode, Act Mode, MCP, custom modes |
...
| Config paths | `.roomodes`, `.roo/rules/*.md` |
| Our generators | `scripts/generate_roo_modes.py`, `scripts/generate_roo_rules.py` |
| Tracked capabilities | `roomodes`, custom modes, Code Actions, MCP, Orchestrator mode |
...
| Tracked capabilities | `.aider.conf.yml`, `CONVENTIONS.md`, architect mode, read, lint-cmd, test-cmd |
...
| Config paths | `.augment/rules/*.md`, `~/.augment/settings.json` |
| Our generators | `scripts/generate_augment.py`, `scripts/generate_augment_rules.py` |
| Tracked capabilities | `.augment`, Agent mode, Next Edit, MCP, context engine |
...
| Config paths | `.agent/rules/*.md`, `.agent/workflows/*.md` |
| Our generators | `scripts/generate_antigravity.py` |
| Tracked capabilities | Antigravity, agent manager, artifacts, MCP, workflows |
...
| Config paths | `AGENTS.md`, `.agents/rules/*.md`, `.codex/hooks.json`, `~/.codex/config.toml` |
| Our generators | `scripts/generate_codex.py`, `scripts/generate_codex_rules.py`, `scripts/generate_codex_hooks.py` |
| Tracked capabilities | `AGENTS.md`, `config.toml`, `mcp_servers`, hooks, sandbox |
...
| Config paths | `opencode.json`, `.opencode/agents/*.md`, `.opencode/commands/*.md`, `.opencode/plugins/*` |
...
| Tracked capabilities | `opencode.json`, agents, commands, plugins, MCP, rules |
