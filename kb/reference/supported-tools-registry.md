...
| Global config paths | Canonical skills `~/.gemini/config/skills/*/SKILL.md`, agents `~/.gemini/config/agents/ai-toolkit-*/agent.md`, hooks `~/.gemini/config/hooks.json` plus adjacent runtime, plugins `~/.gemini/config/plugins/<plugin>/plugin.json`, MCP `~/.gemini/config/mcp_config.json`. `~/.gemini/antigravity-cli/skills/*/SKILL.md` is emitted only for compatibility. Rules stay project-local; `~/.gemini/GEMINI.md` remains the Gemini-CLI-shared instruction surface. |
...
| Current versions / notes | Antigravity 2.0 v2.8.1, Antigravity CLI v1.1.14, IDE v2.5.5 (verified 2026-08-19). Hooks use `PreToolUse`, `PostToolUse`, `PreInvocation`, `PostInvocation`, and `Stop`; the shell tool is `run_command` with the command at `.toolCall.args.CommandLine`. The adapter returns native camelCase payloads, object-shaped `injectSteps`, empty default `terminationBehavior`, and a `decision` string for Stop while preventing re-entry loops. MCP input `url` is accepted only as migration input and emitted as `serverUrl`; `httpUrl` is rejected. Optional `args`, `env`, `cwd`, `headers`, `authProviderType`, `oauth`, `disabled`, and `disabledTools` are preserved. |
