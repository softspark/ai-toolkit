...
| `@softspark/dsh-orchestrator` | `1.0.0` | Claude Code and GitHub Copilot Gemini delegation bundle plus released preset |
...
Mutations hold a bounded DSH lifecycle lock and use the shared state lock with compare-and-swap publication. Install, update, and uninstall verify the profile manifest, package trees, preset identity, and unrelated dependencies before and after each external package-manager command. Rollback restores the immutable pre-operation target. It does not reinterpret concurrent bytes as owned data.

An identity conflict preserves the conflicting path and creates a doctor-visible recovery marker instead of deleting or replacing it. User plugins, dependencies, presets, profile patches, and unrelated state keys remain outside ai-toolkit ownership. Secure mutation requires Linux, WSL, or macOS filesystem primitives. Unsupported platforms fail before the first write.

Profile lifecycle `--dry-run` is read-only. It does not create state, acquire a lock, start DSH, invoke the plugin manager, or create profile paths.
...
`ai-toolkit dsh doctor` is read-only. It reports the DSH runtime version, package pins, package-tree and preset ownership, state consistency, lifecycle lock recovery artifacts, staging paths, and recovery markers. A recovery marker keeps `Recovery needed: yes` visible until the operator resolves the named paths.
