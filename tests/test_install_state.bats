...
@test "install_state: state writers reject a user-owned symlinked state root" {
...
@test "install_state: portable writers survive unsupported secure state primitives" {
    run python3 - "$TOOLKIT_DIR" <<'PY'
import json
import sys
from pathlib import Path
...
@test "install_state: secure state lock acquisition uses a pinned parent" {
    run python3 - "$TOOLKIT_DIR" <<'PY'
import sys
from pathlib import Path
...
try:
    install_state.record_mcp_template("pinned-parent")
finally:
    install_state.os.open = real_open

assert lock_calls, "secure state writer did not acquire the shared state lock"
PY
