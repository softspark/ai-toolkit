...
@test "global uninstall archives foreign files in a recovery session before removal" {
    local repository="$TEST_HOME/.softspark/ai-toolkit/sessions/repo-a"
    local session_directory
    ...
    local archive
    archive="$(ls "$TEST_HOME"/ai-toolkit-backup-*.tar.gz)"
    local member="ai-toolkit/sessions/repo-a/output-filter/$(basename "$session_directory")/keep.txt"
    [ "$(tar -xzOf "$archive" "$member")" = "keep user data" ]
    [ -z "$(tar -tzf "$archive" | grep 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa.json$')" ]
}

@test "foreign-only recovery namespace is archived once, then nothing is left" {
    local repository="$TEST_HOME/.softspark/ai-toolkit/sessions/repo-a"
    ...
    [[ "$output" != *"Recovery:"* ]]
    [[ "$output" == *"Archived: "* ]]
    [ ! -e "$TEST_HOME/.softspark" ]
    [ "$(tar -xzOf "$TEST_HOME"/ai-toolkit-backup-*.tar.gz \
        "ai-toolkit/sessions/repo-a/output-filter/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa/keep.txt")" \
        = "keep user data" ]
