...
def _apply(target_dir: Path, *, dry_run: bool) -> int:
    ...
    pointer_dir = target / ".gemini" / "skills" / POINTER_SKILL_NAME
    ...
    return apply_owned_edits(
        edits,
        target,
        label="Gemini skill pointer",
        prune=(pointer_dir, pointer_dir.parent, target / ".gemini"),
        dry_run=dry_run,
    )


def discover(target_dir: Path) -> int:
    """Return 1 when the managed Gemini catalogue pointer is present."""
    return _apply(target_dir, dry_run=True)


def cleanup(target_dir: Path) -> int:
    """Remove only the managed Gemini catalogue pointer."""
