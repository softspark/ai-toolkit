...
delegation primitives are rendered into Codex-specific wrappers. Generated
variants retain shared references and assets while using Codex guidance.
"""
...
# Wrapper markers written by releases with DSH support; recognised, never written.
LEGACY_DSH_ADAPTED_MARKER = ".ai-toolkit-dsh-adapted"
LEGACY_SHARED_ADAPTED_MARKER = ".ai-toolkit-shared-adapted"
SKILL_SURFACE_OWNERS_MARKER = ".ai-toolkit-skill-owners"
ADAPTED_MARKERS = frozenset({
    ADAPTED_MARKER,
    LEGACY_DSH_ADAPTED_MARKER,
    LEGACY_SHARED_ADAPTED_MARKER,
})
SKILL_SURFACE_OWNERS = frozenset({"codex"})
# Surface owner written by releases with DSH support; recognised, never written.
LEGACY_SKILL_SURFACE_OWNERS = frozenset({"dsh"})
_ADAPTED_MARKER_TEXT = "generated by ai-toolkit for codex\n"
...
def serialize_skill_surface_owners(owners: set[str]) -> bytes:
    ...
    return _encode_skill_surface_owners(owners)


def _encode_skill_surface_owners(owners: set[str]) -> bytes:
    return ("\n".join(sorted(owners)) + "\n").encode("utf-8")
...
def parse_skill_surface_owners(data: bytes) -> set[str] | None:
    ...
    Legacy owners are accepted so an older install stays recognisable as
    toolkit-managed until the next install, update or uninstall converges it.
    """
    try:
    ...
    if len(lines) != len(owners):
        return None
    if not owners or not owners <= SKILL_SURFACE_OWNERS | LEGACY_SKILL_SURFACE_OWNERS:
        return None
    return owners if data == _encode_skill_surface_owners(owners) else None
...
def has_legacy_dsh_skill_surface(target_dir: Path) -> bool:
    """True when `.agents/skills` is owned only by the retired DSH target."""
    owners = skill_surface_owners(target_dir / ".agents" / "skills")
    return bool(owners) and owners <= LEGACY_SKILL_SURFACE_OWNERS


def remove_legacy_dsh_skill_surface(target_dir: Path, skills_src: Path) -> int:
    """Remove the managed part of a DSH-only `.agents/skills` surface.

    User-owned entries stay. The owners marker is removed only once no managed
    entry remains, so a partly removed surface is still recognised later.
    Returns the number of managed skill entries removed.
    """
    if not has_legacy_dsh_skill_surface(target_dir):
        return 0
    agents_dir = target_dir / ".agents"
    skills_dst = agents_dir / "skills"
    _assert_safe_skill_roots(agents_dir, skills_dst)
    marker = agents_dir / SKILL_SURFACE_OWNERS_MARKER
    marker_state = _read_owner_marker_state(marker)
    removed = 0
    is_partial = False
    if skills_dst.is_dir():
        for item in sorted(skills_dst.iterdir()):
            if not _is_managed_entry(item, skills_src):
                continue
            if item.is_symlink():
                item.unlink()
            elif not _remove_managed_wrapper(item, skills_src.resolve()):
                is_partial = True
                continue
            removed += 1
        try:
    ...
    if is_partial:
        return removed
    _assert_owner_marker_state(marker, marker_state)
    marker.unlink()
...
def _sync_adapted_skill(skill_dir: Path, skills_dst: Path) -> str:
    ...
    if _adapted_marker(target) != ADAPTED_MARKER:
        # A legacy DSH or shared wrapper converges on the Codex rendering.
        return _transition_adapted_wrapper(skill_dir, target)

    return _update_adapted_wrapper(skill_dir, target)


def _transition_adapted_wrapper(skill_dir: Path, target: Path) -> str:
    ...
    next_marker = target / ADAPTED_MARKER
    ...
    next_marker_file = _stage_text(next_marker, _ADAPTED_MARKER_TEXT)
    try:
        ...
        if next_marker.is_file() and not next_marker.is_symlink():
            if next_marker.read_bytes() == _ADAPTED_MARKER_TEXT.encode("utf-8"):
                next_marker.unlink()
...
def _create_adapted_wrapper(
    ...
    *,
    ...
    try:
        ...
        _write_text_fsync(staging / ADAPTED_MARKER, _ADAPTED_MARKER_TEXT)
...
def _adapt_body(body: str, platform: str) -> str:
    ...
    subagents = f"{label}-native subagents"
