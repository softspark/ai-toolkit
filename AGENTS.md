...
# AI Toolkit — opencode Configuration

Shared AI development toolkit with specialized subagents, opencode-compatible commands, quality hooks, and a safety constitution.

opencode reads this file at session start. Subagents are also installed under `.opencode/agents/` and slash commands under `.opencode/commands/` for native `@` and `/` autocomplete.

## Available Subagents

Invoke with `@<name>` in opencode, or delegate via `spawn_agent`:
...
Invoke with `/<name>` in opencode. Knowledge-only skills load automatically:
